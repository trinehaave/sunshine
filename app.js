...
//set the colors to be used for background linear gradient
	...
	$('body').css({background: "linear-gradient(#" + state.currentTopColor + ", #" + state.currentBottomColor +")"}).css({height: "100%"}).css({margin: "0"}).css({"background-repeat": "no-repeat"}).css({"background-attachment": "fixed"});
	$('span').css({color: '#fff'}).css({'letter-spacing': '0px'}).css({background: "#" + state.currentBottomColor}).css({padding: '5px'});
  	$('.search-again').css({color: '#' + state.currentBottomColor});
